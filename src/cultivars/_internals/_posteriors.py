...
class _SVPosterior(_SummaryMixin, _SeriesMixin):
    ...
    @classmethod
    def _from_gibbs(cls, fit: _VolatilityDrawsFit, model: SV) -> SVPosterior:
    ...
    @classmethod
    def _from_chain(
        cls, fit: _ParticleChainFit, mean: float, model: SV, *, n_particles: int, seed: int | None
    ) -> SVPosterior:
        ...
        params = _StochasticVolatilityParameters(
            mu=float(mu.mean()), phi=float(phi.mean()), sigma2=float(sigma2.mean()), mean=mean
        )
    ...
    def _summary_table(self) -> SummaryTable:
        ...
        if self.acceptance_rate is not None:
            metadata.append(("Acceptance", f"{self.acceptance_rate:.3f}"))
            metadata.append(("Thin", f"{self.thin}"))
        return SummaryTable(
            title=f"SV[{self.mean_spec}] Posterior",
...
class _UCSVPosterior(_SummaryMixin, _SeriesMixin):
    ...
    @classmethod
    def _from_fit(cls, fit: _TrendVolatilityFit, model: UCSV) -> UCSVPosterior:
...
class _PerturbationDSGEPosterior(_SummaryMixin):
    ...
    @classmethod
    def _from_fit(
        cls, fit: _ParticleChainFit, model: PerturbationDSGE
